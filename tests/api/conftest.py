"""A real aiohttp server to test the client against.

The plan called for `aioresponses`, but its current release (0.7.9) is
incompatible with aiohttp 3.14, which Home Assistant pins - it fails with
"ClientResponse.__init__() missing 1 required keyword-only argument".

Driving a real local server is a better trade anyway: status codes, 204
bodies, Date headers and JSON decoding all behave exactly as they will in
production, instead of as the mock library imagines.
"""
